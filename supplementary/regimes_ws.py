...
k_ws_values = [4, 6, 8, 10, 12]
beta_values = np.linspace(0.0, 0.6, 7)
...
def worker_task(params):
    exp, k_ws, beta, k1, k2, sigma = params
    n_steps, i_w, i_s, w_s, ni, nw, ns = run_simulation(k1, k2, sigma)
    return [
        k1, k2, sigma, k_ws, beta, exp, n_steps,
...
if __name__ == '__main__':
    ...
    num_experiments = 30
    csv_filename = 'regimes_results_ws.csv'
    with open(csv_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            'k1','k2','sigma','k_ws','beta','experiment','n_steps',
        ...
        for exp in range(1, num_experiments+1):
            ...
            for k_idx, k_ws in enumerate(k_ws_values, start=1):
                if k_ws % 2 != 0:
                    raise ValueError(f"k_ws must be even; got {k_ws}")
                for b_idx, beta in enumerate(beta_values, start=1):
                    print(f"Exp {exp}: k={k_ws} beta {b_idx}/{len(beta_values)}")
                    graph_seed = exp * 10000 + k_idx * 100 + b_idx
                    G = nx.watts_strogatz_graph(n, k_ws, float(beta), seed=graph_seed)
                    ...
                    for initial in initials:
                        with multiprocessing.Pool(
                            ...
                            tasks = [
                                (exp, k_ws, float(beta), k1, k2, float(sigma))
                                for k1 in k1_values
