...
#     i_s            = col_character(),
#     w_s            = col_character(),
#     n_inactive     = col_character(),
#     n_weak         = col_character(),
#     n_strong       = col_character()
#   )
# ) %>%
#   mutate(
#     across(
#       i_w:n_strong,
#       ~ map(.x, ~ as.integer(unlist(fromJSON(.x))))
#     )
#   )
# 
# saveRDS(df_parsed, file = "regimes_results_ws_parsed.rds")
```

```{r}
df <- readRDS("regimes_results_ws_20_parsed.rds")
```
...
plot_single_trajectory <- function(data,
  ...
  if (metric == "p") {
    ...
    p <- ggplot() +
      ...
      labs(
        title = paste0("k1=",k1_val,", k2=",k2_val,", σ=",sigma_val,
                       ", k_ws=",k_ws_val,", β=",beta_val),
